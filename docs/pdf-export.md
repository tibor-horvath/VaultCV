...
Page splits use vertical slice positions aligned to **`data-pdf-page-break`** markers in the print DOM. Experience, projects, and education add markers on sub-blocks (date row, each company link row, bullet lists, etc.) so a cut is less likely to fall between the website line and the LinkedIn line when a job is taller than one printed page.
