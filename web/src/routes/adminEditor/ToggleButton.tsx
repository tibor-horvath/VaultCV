...
export function ToggleButton(props: { pressed: boolean; onClick: () => void; title?: string; label?: string; pressedLabel?: string; unpressedLabel?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 rounded-field border px-2 py-1 text-[11px] font-semibold ${
        pressed
          ? 'border-positive/40 bg-positive-soft text-positive-soft-ink hover:bg-positive-soft'
          : 'border-red-300/70 bg-critical-soft text-critical-soft-ink hover:bg-red-100'
      }`}
      title={props.title ?? (pressed ? pressedLabel : unpressedLabel)}
    >
      {pressed ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
      {label ? <span className="text-ink-muted/80">{label}</span> : null}
