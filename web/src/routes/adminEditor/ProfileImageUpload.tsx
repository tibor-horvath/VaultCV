...
function CropModal({ imageFile, onConfirm, onCancel }: CropModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={t('adminCropProfilePhoto')}>
      <div className="flex w-full max-w-xs flex-col gap-4 rounded-card bg-surface p-5 shadow-overlay">
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-ink">{t('adminCropPhoto')}</span>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div
          ref={containerRef}
          className="relative aspect-square w-full cursor-grab overflow-hidden rounded-full border-2 border-accent active:cursor-grabbing"
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 accent-[rgb(var(--vc-accent))]"
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 rounded-field border border-line bg-surface px-3 py-2 text-xs font-medium text-ink-muted hover:bg-surface-muted"
          >
            {t('adminCancel')}
          </button>
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded-field bg-accent px-3 py-2 text-xs font-semibold text-accent-ink hover:bg-accent-hover"
          >
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3">
        {/* Circular preview */}
        <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-full border border-line bg-surface-muted">
          {previewImage ? (
            <canvas
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="rounded-field border border-line bg-surface px-2.5 py-1.5 text-xs font-medium text-ink-muted hover:bg-surface-muted disabled:opacity-50"
            >
            ...
            {hasProfileImage ? (
              <button
                ...
                className="rounded-field border border-critical/25 bg-surface px-2 py-1.5 text-xs font-medium text-critical hover:bg-critical-soft disabled:opacity-50"
          ...
          <p className="text-[11px] text-ink-subtle">{t('adminProfilePhotoRequirements')}</p>
          {uploadError ? <p className="text-[11px] text-critical-soft-ink">{uploadError}</p> : null}
        </div>
      </div>
